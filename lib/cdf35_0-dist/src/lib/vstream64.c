...
static int OpenFile PROTOARGs((char *file_spec, char *a_mode));
#else
...
******************************************************************************/
#if defined(win32) && (_FILE_OFFSET_BITS == 64)
static int OpenFile (file_spec, a_mode)
#else
  ...
  if (vFp->fp == NULL) {
    ...
    for (i = 1; i <= MAX_TMP; i++) {
       ...
       snprintf (EofS(tmpPath), (size_t) pathLength + 1 - strlen(tmpPath),
                 "TMP%05ld.%s", i, vFp->scratchExt);
       if (!IsReg(tmpPath)) {
#if defined(win32) && (_FILE_OFFSET_BITS == 64)
         fh = OpenFile (tmpPath, WRITE_PLUS_a_mode);
         if (fh == -1) {
#else
...
/*
  tmpBlockN = ((OFF_T) (vFp->offset64 + nBytes - 1)) / nCACHE_BUFFER_BYTEs;
  if (tmpBlockN > maxCount) lastBlockN = 0;
  else lastBlockN = (long) tmpBlockN;
*/
...
vFILE *vFp;             /* Pointer to vFILE structure. */
{
  ...
  ****************************************************************************/
  firstBlockN = (long) (vFp->offset64 / (OFF_T) nCACHE_BUFFER_BYTEs); 
/*
  tmpBlockN = (OFF_T) (vFp->offset64 / nCACHE_BUFFER_BYTEs);
  if (tmpBlockN > maxCount) firstBlockN = 0;
  else firstBlockN = (long) tmpBlockN; 
*/
  bufferOffset = (int) ((OFF_T) vFp->offset64 % nCACHE_BUFFER_BYTEs);
  ...
  ****************************************************************************/
  lastBlockN = (long) ((vFp->offset64 + (OFF_T) nBytes - 1) / 
			(OFF_T) nCACHE_BUFFER_BYTEs); 
/*  tmpBlockN = ((OFF_T) (((OFF_T)vFp->offset64) + nBytes - 1)) / 
			nCACHE_BUFFER_BYTEs;
  if (tmpBlockN > maxCount) lastBlockN = 0;
  else lastBlockN = (long) tmpBlockN;
*/
