...
#endif
{
  ...
  for (;;) {
	 ...
	 if (!Write32_64(fp,&(GDR->rfuD))) return CWE;
  ...
  for (;;) {
	 ...
	 if (STRINGdataType(dataType)) {
	   ...
	   if ((int)len >= numElems) {
	     ...
	     strncpy (newPad, (char *)padValue, len);
	     for (ix = (int) len; ix < numElems; ++ix)
		*(newPad+ix) = (char) ' ';
	     if (!SEEKv64(fp,tOffset,vSEEK_SET)) return CWE;
	     if (!sX(WriteBuffer64(CDF,fp,dataType,
		     numElems,newPad),&pStatus)) return pStatus;
	     cdf_FreeMemory (newPad, NULL);
	   }
	 } else {
	   if (!SEEKv64(fp,tOffset,vSEEK_SET)) return CWE;
	   if (!sX(WriteBuffer64(CDF,fp,dataType,
