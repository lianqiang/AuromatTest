...
STATICforIDL CDFstatus CDFsel64 (Va, Cur)
struct VAstruct *Va;
...
switch (Va->item) {
  ...
  ****************************************************************************/
  case CDF_READONLY_MODE_: {
    ...
    /*************************************************************************
    * Clear the READONLY metadata state info and read in metatdata if
    * READONLYon.
    *************************************************************************/
    if (Change)
    {
        ResetReadOnlyState64(CDF);
  ...
  ****************************************************************************/
  case CDF_zMODE_: {
    ...
    if (!sX(ConfigureNEWzMode64(CDF),&pStatus)) {
      AbortAccess64 (CDF, UPDATE, noDELETE);
      return pStatus;
    };
    break;
  ...
  ****************************************************************************/
  case COMPRESS_CACHESIZE_: {
	...
	return BAD_CACHE_SIZE;
      ...
      if (!CACHEv64(CDF->compressFp,CDF->compressCacheSize)) {
	AbortAccess64 (CDF, UPDATE, noDELETE);
	return BAD_CACHE_SIZE;
      }
    }
  ...
  ****************************************************************************/
  ...
  case zVARs_CACHESIZE_: {
    ...
    if (zModeON(CDF)) {
	 ...
	 if (!sX(InitVar64(CDF,varN,FALSE,&Var),&pStatus)) {
	   AbortAccess64 (CDF, UPDATE, noDELETE);
	   return pStatus;
	 }
	 ...
	 if (!sX(InitVar64(CDF,varN,TRUE,&Var),&pStatus)) {
	   AbortAccess64 (CDF, UPDATE, noDELETE);
	   return pStatus;
	 }
	 Var->varCacheSize = nBuffers;
	 ...
	 if (!sX(InitVar64(CDF,varN,zOp,&Var),&pStatus)) {
	   AbortAccess64 (CDF, UPDATE, noDELETE);
	   return pStatus;
	 }
	 Var->varCacheSize = nBuffers;
  ...
  ****************************************************************************/
  ...
  case zVAR_: {
	...
	if (CDF->CURzVarNum == varNum) {
	  offset = CDF->CURzVarOffset64;
	  return CDF_OK;
	} else {
          if (!sX(FindVarByNumber64(CDF,varNum,FALSE,&offset),&pStatus)) {
	    AbortAccess64 (CDF, UPDATE, noDELETE);
	    return pStatus;
	  }
	...
	if (varNumT < CDF->NzVars) {
	  if (CDF->CURzVarNum == varNum) {
	    ...
	    return CDF_OK;
            ...
            if (!sX(FindVarByNumber64(CDF,varNumT,TRUE,&offset),&pStatus)) {
	      AbortAccess64 (CDF, UPDATE, noDELETE);
	      return pStatus;
	    }
	    CDF->CURzVarNum = varNum;
	...
	if (zOp) {
	  if (CDF->CURzVarNum == varNum) {
	    ...
	    return CDF_OK;
            ...
            if (!sX(FindVarByNumber64(CDF,varNum,TRUE,&offset),&pStatus)) {
	      AbortAccess64 (CDF, UPDATE, noDELETE);
	      return pStatus;
	    }
	    CDF->CURzVarNum = varNum;
  ...
  ****************************************************************************/
  ...
  case zVAR_SEQPOS_: {
    ...
    Var->seqValueOffset64 = BOO(Var->recVary,recNumber*Var->NphyRecValues,0);
    Var->seqValueOffset64 += IndicesValueOffset (Var->numDims, indices,
					         Var->dimVarys,
					         Var->nPhyDimValues);
    break;
  }
  ...
  ****************************************************************************/
  ...
  case ATTR_NAME_: {
    ...
    long attrNum; char *attrName; struct CDFstruct *CDF; OFF_T offset;
    if (nameOp)
    ...
    **************************************************************************/
    SelectCDF (Cur->cdf, CDF)
    tStatus = BOO(nameOp,FindAttrByName64(CDF,attrName,&offset),
			 FindAttrByNumber64(CDF,(Int32)attrNum,&offset));
    switch (tStatus) {
      case CDF_OK:
  ...
  ****************************************************************************/
  ...
  case zENTRY_: {
    ...
    **************************************************************************/
    if (!sX(BOO(zOp,SetCURzEntry64(CDF,TRUE,(Int32)entryNum),
		    SetCURgrEntry64(CDF,TRUE,(Int32)entryNum)),&pStatus)) {
      AbortAccess64 (CDF, UPDATE, noDELETE);
      return pStatus;
    }
    break;
  ...
  ****************************************************************************/
  ...
  case zENTRY_NAME_: {
    ...
    **************************************************************************/
    if (!sX(BOO(zOp,SetCURzEntry64(CDF,FALSE,(Int32)entryNum),
		    SetCURgrEntry64(CDF,FALSE,(Int32)entryNum)),&pStatus)) {
      AbortAccess64 (CDF, UPDATE, noDELETE);
      return pStatus;
    }
    break;
