...
#if (!SUPPORT_HUFF64 || !SUPPORT_AHUFF64) && defined(BORLANDC)
#pragma warn -par
#endif
...
typedef uInt4 INT;       /* Needed to pass a WORD by value. */
...
#if SUPPORT_HUFF64 || SUPPORT_AHUFF64
...
static Logical count_bytes PROTOARGs((
  vFILE *input, DWORD *counts, OFF_T iSize
));
...
static CDFstatus compress_data PROTOARGs((
  vFILE *input, BIT_FILE *output, CODE *codes, OFF_T iSize, CDFstatus iError,
  CDFstatus oError
));
...
STATICforIDL CDFstatus CompressHUFF0_64 (input, iOffset, iSize, iError,
...
CDFstatus oError;
{
  ...
  CDFstatus pStatus = CDF_OK; int root_node; OFF_T newOffset;
  if (!SEEKv64(input,iOffset,vSEEK_SET)) return iError;
...
STATICforIDL CDFstatus DecompressHUFF0_64 (iFp, iOffset, iError,
...
CDFstatus oError;
{
  ...
  if (!SEEKv64(output,oOffset,vSEEK_SET)) return oError;
...
STATICforIDL CDFstatus CompressAHUFF0_64 (input, iOffset, iSize, iError,
				          oFp, oOffset, oSize, oError)
vFILE *input;
OFF_T iOffset;
OFF_T iSize;
...
STATICforIDL CDFstatus DecompressAHUFF0_64 (iFp, iOffset, iError,
					    output, oOffset, oError)
vFILE *iFp;
OFF_T iOffset;
CDFstatus iError;
 ...
 */
	if (V_putc64(first,output->file) != first) return FALSE;
	if (V_putc64(last,output->file) != last) return FALSE;
	for ( i = first ; i <= last ; i++ ) {
	    if (V_putc64((int) nodes[ i ].count, output->file ) !=
		 (int) nodes[ i ].count ) return FALSE;
	}
    }
    if (V_putc64(0,output->file) != 0) return FALSE;
    return TRUE;
}
#endif
...
OFF_T iSize;
{
    ...
    input_marker = V_tell64 (input);
    if (input_marker == EOF) return FALSE;

    for (i = 0; i < iSize; i++) {
      if ((c = V_getc64(input)) == EOF) return FALSE;
      counts[c]++;
    }

    if (!SEEKv64(input,input_marker,vSEEK_SET)) return FALSE;
    return TRUE;
}
#endif
...
CDFstatus oError;
{
    ...
    for (i = 0; i < iSize; i++) {
       if ((c = V_getc64(input)) == EOF) return iError;
...
#if SUPPORT_HUFF6464 || SUPPORT_AHUFF64
/******************************************************************************
* StartBitFile.
******************************************************************************/
...
int count;
{
	...
	bit_file->mask >>= 1;
	if ( bit_file->mask == 0 ) {
	    if (V_putc64( bit_file->rack, bit_file->file ) != bit_file->rack ) {
		return FALSE;
	    }
	    bit_file->rack = 0;
	    bit_file->mask = 0x80;
...
BIT_FILE *bit_file;
{
    int value;
    if ( bit_file->mask == 0x80 ) {
	bit_file->rack = V_getc64( bit_file->file );
...
NODE *nodes;
{
    ...
    if ( ( first = V_getc64( input->file ) ) == EOF ) return FALSE;
    if ( ( last = V_getc64( input->file ) ) == EOF ) return FALSE;
    for ( ; ; ) {
	for ( i = first ; i <= last ; i++ ) {
	    if ( ( c = V_getc64( input->file ) ) == EOF ) return FALSE;
...
CDFstatus oError;
{
	...
	if ( ( V_putc64( node, output ) ) != node ) return oError;
    }
    return CDF_OK;
}
#endif
