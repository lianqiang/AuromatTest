...
C-----------------------------------------------------------------------
        ...
        INCLUDE 'CDF.INC'
        INTEGER (KIND=KIND_INT8) uSizeOUT8, cSizeOUT8
	...
	INTEGER*4 ID, STATUS, ENCODING, MAJORITY, NUM_DIMS,
     ...
     .		  I, X1, X2, X, decodingOut,
     .		  VAR_REC_VARIANCE, VAR_REC_VARIANCE_OUT, recnum_out,
     .		  VAR_DIM_VARIANCES(2), reccount_out, cSizeOUT4,
     ...
     .		  dimintervals_out(CDF_MAX_DIMS), cParms(CDF_MAX_PARMS)
        INTEGER*4 cParmsOUT(CDF_MAX_PARMS), cPctOUT, uSizeOUT4,
	...
	DATA ENCODING/IBMPC_ENCODING/, MAJORITY/COL_MAJOR/,
     ...
     .	     ENTRY_VALUE/1/, numVars/2/,
     .	     VAR_VALUES/'11111','22222','33333',
     .			'44444','55555','66666'/,
     .	     CDFNAME(1:4)/'TEST'/, VARNAME(1:4)/'VAR1'/,
     .	     NEW_VARNAME(1:4)/'VAR2'/, ATTRNAME(1:5)/'ATTR1'/,
     .	     NEW_ATTRNAME(1:5)/'ATTR2'/, cType/GZIP_COMPRESSION/,
     .	     cParms(1)/9/, sRecordsType/PAD_SPARSERECORDS/,
...
C	CALL CDF_SET_FILEBACKWARD(BACKWARDFILEon)
C       CALL CDF_SET_VALIDATE(VALIDATEFILEon)
...
C-----------------------------------------------------------------------
        ...
        STATUS = CDF_LIB    (PUT_, CDF_CHECKSUM_, CHECKSUM,
     .                       NULL_, STATUS)
...
C-----------------------------------------------------------------------
	...
	IF ((Flag .EQ. 0) .AND. (VERSIONcdf .NE. VERSIONlib))
     .	  CALL QUIT_CDF (STATUS, '15.2')
	IF ((Flag .EQ. 0) .AND. (RELEASEcdf .NE. RELEASElib))
     .	  CALL QUIT_CDF (STATUS, '15.3')
	IF ((Flag .EQ. 0) .AND. (INCREMENTcdf .NE. INCREMENTlib))
     .	  CALL QUIT_CDF (STATUS, '15.4')
	IF (COPYRIGHT_TEXTcdf .NE. COPYRIGHT_TEXTlib)
     .	  CALL QUIT_CDF (STATUS, '15.4')

        STATUS = CDF_LIB    (GET_, CDF_CHECKSUM_, CHECKSUM_out,
     .                       NULL_, STATUS)
        IF (STATUS .LT. CDF_OK) CALL QUIT_CDF (STATUS, '15.5')

        IF (CHECKSUM_out .NE. CHECKSUM)
     .    CALL QUIT_CDF (STATUS, '15.6')
...
C	IF (READONLYmode .NE. READONLYon)
C     .    CALL QUIT_CDF (STATUS, '26.2')
