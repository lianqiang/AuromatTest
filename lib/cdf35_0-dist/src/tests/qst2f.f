...
C-----------------------------------------------------------------------
	...
	INTEGER*4 I
	INTEGER*4 LAST_CHAR
        INTEGER*4 CHECKSUM, CHECKSUM_OUT
...
C					! (before padding blanks begin)
	...
	CHARACTER VARNAME*(CDF_VAR_NAME_LEN256)
	CHARACTER NEW_VARNAME*(CDF_VAR_NAME_LEN256)
	CHARACTER VARNAME_OUT*(CDF_VAR_NAME_LEN256)
	CHARACTER ATTRNAME*(CDF_ATTR_NAME_LEN256)
	CHARACTER NEW_ATTRNAME*(CDF_ATTR_NAME_LEN256)
	CHARACTER ATTRNAME_OUT*(CDF_ATTR_NAME_LEN256)
	...
	CHARACTER EPSTRING_TRUE*(EPOCH_STRING_LEN),
     ...
     .		  EPSTRING4_TRUE*(EPOCH4_STRING_LEN)

	DATA ENCODING/IBMPC_ENCODING/
	DATA MAJORITY/COL_MAJOR/
        DATA CHECKSUM/MD5_CHECKSUM/
	...
	DATA EPSTRING_TRUE/'13-Oct-1994 12:00:00.000'/,
     ...
     .	     EPSTRING4_TRUE/'1994-10-13T12:00:00.000'/
...
C-----------------------------------------------------------------------
C  Set checksum (to MD5).
C-----------------------------------------------------------------------

        CALL CDF_SET_CHECKSUM (CDF_ID, CHECKSUM, STATUS)
        IF (STATUS .LT. CDF_OK) CALL QUIT_CDF (STATUS, '6.0')
...
C	IF (ENCODING_OUT .NE. ENCODING)
C    .	  CALL QUIT_CDF (STATUS, '6a.3')
...
C-----------------------------------------------------------------------
	...
	CALL CDF_VAR_HYPER_PUT (CDF_ID, CDF_VAR_NUM(CDF_ID,VARNAME),
...
C-----------------------------------------------------------------------
	...
	CALL CDF_VAR_HYPER_GET (CDF_ID, CDF_VAR_NUM(CDF_ID,VARNAME),
     .				REC_START, REC_COUNT, REC_INTERVAL,
     .				INDICES, COUNTS, INTERVALS,
...
C									! V1.1
	...
	IF (NUM_ATTRS_OUT .NE. 1) CALL QUIT_CDF (STATUS, '16.7')
        ...
        CALL CDF_GET_CHECKSUM (CDF_ID, CHECKSUM_OUT, STATUS)
        IF (STATUS .LT. CDF_OK) CALL QUIT_CDF (STATUS, '16.8')
        IF (CHECKSUM_OUT .NE. CHECKSUM) CALL QUIT_CDF (STATUS, '16.9')
...
C-----------------------------------------------------------------------
	...
	CALL CDF_VAR_INQUIRE (CDF_ID, CDF_VAR_NUM(CDF_ID,NEW_VARNAME),
...
C-----------------------------------------------------------------------
	...
	CALL CDF_ATTR_RENAME (CDF_ID, CDF_ATTR_NUM(CDF_ID,ATTRNAME),
     .			      NEW_ATTRNAME, STATUS)
...
C-----------------------------------------------------------------------
	...
	CALL ENCODE_EPOCH4 (EPOCH, EPSTRING4)
	IF (EPSTRING4 .NE. EPSTRING4_TRUE) CALL QUIT_EPOCH ('30.8')

	CALL PARSE_EPOCH4 (EPSTRING4, EPOCH_OUT)
	IF (EPOCH_OUT .NE. EPOCH) CALL QUIT_EPOCH ('30.9')
