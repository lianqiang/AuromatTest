...
static int lineAttrs = 0, lineEntries;
  ...
  for (icount = 0; icount < numCDFs; icount++) {
    ...
    ***************************************/
    ...
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     NULL_);
    if (status < CDF_OK) QuitCDF ("3.0", status, "Parent");
    ...
    if (status < CDF_OK || numigEntries < 1) {
      ...
      if (status < CDF_OK || numigEntries < 1) {
        status = CDFlib (SELECT_, CDF_, ido,
                         ...
                         NULL_);
        if (status < CDF_OK) QuitCDF ("9.0", status, "Parent");
        ++numgEntries;
      }
    }
    /********************************************
        ...
        if (strcmpIgCase (varName, "range_epoch") == 1) {
          if (first) {
            ...
            *******************************************************/
            ...
            if (status < CDF_OK) QuitCDF ("10.2", status, "Epoch or RANGE_EPOCH");
...
/*
            status = CDFlib (SELECT_, CDF_, ido,
                             CREATE_, zVAR_, "VirtualEpoch", dataType, 1L, 0L,
                                             dimSizes, VARY, dimVarys, &varNum,
                             NULL_);
            if (status < CDF_OK) QuitCDF ("10.4", status, "VirtualEpoch");
*/
        ...
        if (status < CDF_OK) {
          if (status == VAR_EXISTS) {
            if (dataOnly || (cdaweb && 
              ...
              QuitCDF ("12.0", status, newVarName);
          } else
...
*/
                    if (strcmpIgCase (varName, "range_epoch") == 1) 
                      QuitCDF ("19.7", status, newVarName);
                  }
                }
              }
              k++;
  ...
  if (augmentlabel) {
    if (!augmentlabelExist) {
      status = CDFlib (SELECT_, CDF_, ido,
                       ...
                       NULL_);
      if (status < CDF_OK) 
        QuitCDF ("20.2", status, "AUGMENT_LABL");
    } else {
      status = CDFlib (SELECT_, CDF_, ido,
                       GET_, ATTR_NUMBER_, "AUGMENT_LABL", &attrNum,
                       NULL_);
      if (status < CDF_OK) 
        QuitCDF ("20.3", status, "AUGMENT_LABL");
    }
    status = CDFlib (SELECT_, CDF_, ido,
                     GET_, CDF_NUMzVARS_, &numVars,
                     NULL_);
    if (status < CDF_OK) 
      QuitCDF ("20.4", status, "AUGMENT_LABL");
    for (i = 0; i < (int) numVars; i++) {
      status = CDFlib (SELECT_, CDF_, ido,
                       ...
                       NULL_);
      if (status < CDF_OK) 
        QuitCDF ("20.6", status, "AUGMENT_LABL");
    }
  }
...
*/
      if (strcmpIgCase (varName, "range_epoch") == 1) {
        ...
        status = CDFlib (SELECT_, zVAR_RECNUMBER_, 0L,
                         ...
                         NULL_);
        if (status < CDF_OK) QuitCDF ("22.3", status, "Epoch or RANGE_EPOCH");
        ...
        if (whichEpoch2 == 1) 
          ...
          status = CDFlib (GET_, zVAR_HYPERDATA_, minmaxTT2000,
                           NULL_);
        if (status < CDF_OK) QuitCDF ("22.5", status, "Epoch or RANGE_EPOCH");
  ...
  **********************************************************************/
  if (whichEpoch2 != 0) {
    ...
    status = CDFlib (SELECT_, CDF_, ido,
                              ...
                              zVAR_DIMINTERVALS_, dimSizes,
                     NULL_);
    if (status < CDF_OK) QuitCDF ("39.3", status, "Epoch or RANGE_EPOCH");
    ...
    if (whichEpoch2 == 1) 
      ...
      status = CDFlib (PUT_, zVAR_HYPERDATA_, minmaxTT2000,
                       NULL_);
    if (status < CDF_OK) QuitCDF ("39.5", status, "Epoch or RANGE_EPOCH");
  }
...
void ConstructOutputFileName2 (name)
char *name;
{ 
  char fileName[DU_MAX_PATH_LEN+1];
  char fileNaming[DU_MAX_PATH_LEN+1];
  ...
  if (lineAttrs == 0 || lineEntries == 0) return;
...
void GetDataTimeStamp (timeStamp)
char *timeStamp;
{ 
  char **varNames;
  char *tmp;
  long numVars, numrVars, numzVars, dataType, numElems;
  ...
  for (ix = 0; ix < numCDFs; ix++) {
    ...
    status =  CDFlib (OPEN_, CDF_, CDFpaths[ix], &idi,
                      NULL_);
    if (status != CDF_OK) QuitCDF ("GetDataTimeStamp", status, CDFpaths[ix]);
    ...
    status =  CDFlib (SELECT_, CDF_, idi,
                      ...
                      GET_, CDF_NUMrVARS_, &numrVars,
                            CDF_NUMzVARS_, &numzVars,
                      NULL_);
    if (status != CDF_OK) QuitCDF ("GetDataTimeStamp", status, "getting numbers of varibales");
    ...
    for (iz = 0; iz < numrVars; iz++) {
      status =  CDFlib (SELECT_, rVAR_, (long) iz,
                        ...
                        NULL_);
      if (status != CDF_OK) QuitCDF ("GetDataTimeStamp", status, "var_name");
...
*/
    ...
    status =  CDFlib (CLOSE_, CDF_, 
                      NULL_);
    if (status != CDF_OK) QuitCDF ("GetDataTimeStamp", status, "closing CDF");
...
void LoadgAttributes (ids, numEntries)
CDFid ids;
int numEntries;
{         

  int i, j, k, ii;
  Logical iFound, fnc = FALSE, lfi = FALSE, lsi = FALSE;
  long numgEntries;
          ...
          if (!ifound) {
            ...
            if (!ifound) {
              ...
              status = CDFlib (GET_, gENTRY_DATA_, valueY,
                               NULL_);
              if (status < CDF_OK) QuitCDF ("7.2", status, NULL);
            ...
            status = CDFlib (SELECT_, CDF_, ido,
                                      ...
                                      gENTRY_, (long) 0,
                             NULL_);
            if (status < CDF_OK) QuitCDF ("7.5", status, newAttrName);
  ...
  if (fnc) {
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     NULL_);
    if (status != CDF_OK) QuitCDF ("8.1", status, "File_naming_convention");
  }
  if (lfi) {
    long numE;                    
    if (EndsWithIgCase (outputCDF, ".cdf"))
    ...
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     PUT_, gENTRY_DATA_, CDF_CHAR, numE,
                           outputCDF,
                     NULL_);
    if (status != CDF_OK) QuitCDF ("8.1a", status, "Logical_file_id");
    printf("  Global attribute: Logical_file_id has been reset to: %*.*s\n",
                                                                  (int)numE,
                                                                  (int)numE,
                                                                  outputCDF);
  ...
  if (lsi) {
    ...
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     PUT_, gENTRY_DATA_, CDF_CHAR, numE,
                           outputCDF,
                     NULL_);
    if (status != CDF_OK) QuitCDF ("8.1b", status, "Logical_source");
  }
...
{         
  ...
  for (i = 0; i < lineAttrs; i++) {
    ...
    for (l = 0; l < ii; l++) {
      ...
      if (status < CDF_OK) QuitCDF ("8.0a", status, attrs[i]);
    }
  }
  if (fnc) {
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     PUT_, gENTRY_DATA_, CDF_CHAR, 26L,
                           "source_descriptor_datatype",
                     NULL_);
    if (status != CDF_OK) QuitCDF ("8.2", status, "File_naming_convention");
  }
  ...
  if (lfi) {
    ...
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     PUT_, gENTRY_DATA_, CDF_CHAR, numE,
                           outputCDF,
                     NULL_);
    if (status != CDF_OK) QuitCDF ("8.2b", status, "Logical_file_id");
  }
  ...
  if (lsi) {
    ...
    status = CDFlib (SELECT_, CDF_, ido,
                     ...
                     PUT_, gENTRY_DATA_, CDF_CHAR, numE,
                           outputCDF,
                     NULL_);
    if (status != CDF_OK) QuitCDF ("8.2c", status, "Logical_source");
  }
}
