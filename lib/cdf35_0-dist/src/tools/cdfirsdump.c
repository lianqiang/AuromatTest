     ...
     *************************************************************************/
     switch (recordType) { 
	  ...
	  if (MOST(level)) {
        ...
        **********************************************************************/
        case CDR_: {
          Int32 version, release, increment, myflag;
          char *docText;
          CDRcount++;
          CDRbytes += recordSize;
	  if (MOST(level)) {
	  ...
	  if (MOST(level)) {
        ...
        **********************************************************************/
        case UIR_: { 
          UIRcount++;
          UIRbytes += recordSize;
          if (16 <= recordSize) {         /* Assumed a post-beta V2.5 CDF. */
	    if (MOST(level)) {
              if (!Read4(CDFfp,&int32)) return;
              snprintf (text, (size_t) sizeof(text),
		        "Next: %s\n", FileOffset(int32));
...
static void DisplaySummary64 (long level) {
  ...
  if (checksum == 1)
    snprintf (text, (size_t) sizeof(text),
	      "\n\n  Total bytes: %*lld (+16 if with checksum)", width1, totalBytes64);
  else
    snprintf (text, (size_t) sizeof(text),
  ...
  if (checksum == 1)
    snprintf (text, (size_t) sizeof(text),
	      "\n\n  Total bytes: %*I64d (+16 if with checksum)", width1, totalBytes64);
  else
    snprintf (text, (size_t) sizeof(text),
  ...
  if (!MOST(level))
    ...
    snprintf (text, (size_t) sizeof(text),
	      ...
	      width1, ADRcount, width2, ADRbytes64, pctADR, gADRcount, vADRcount);
#else
  if (!MOST(level))
    snprintf (text, (size_t) sizeof(text),
...
OFF_T value;
{
  ...
  which = (which + 1) % 3;
#if !defined(win32) && !defined(__MINGW32__)
  snprintf (text[which], (size_t) 21, "%020lld", (OFF_T) value);
#else
  snprintf (text[which], (size_t) 21, "%020I64d", (OFF_T) value);
#endif
  return text[which];
}
...
OFF_T value;
{
  ...
  static int which = -1;
  which = (which + 1) % 3;
  snprintf (text[which], (size_t) 21, Int64hexFORMAT, value);
  return text[which];
}
