...
char *argV[];
{
  ...
  static int optRequired[] = {
    ...
    TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, 0 };
    ...
    if (version >= 3) {
      ...
      if ((cSize >= (OFF_T) ((1LL << 31) - 1)) ||
        ...
        snprintf (text, (size_t) sizeof(text),
		  "cSize(%I64d) uSize(%I64d)", cSize, uSize);
#else
        snprintf (text, (size_t) sizeof(text),
		  "cSize(%lld) uSize(%lld)", cSize, uSize);
#endif
...
/*			  VAR_COMPRESSION(Z), cType, cParms, */
   ...
   if (!recVary) {
     ...
     if ((size*values) >= 1024) { /* Allow compression if NRV record >= 1K */
       ...
       long cP[] = {0};
       status = CDFlib (PUT_, VAR_COMPRESSION(Z), NO_COMPRESSION, cP,
                        NULL_);
