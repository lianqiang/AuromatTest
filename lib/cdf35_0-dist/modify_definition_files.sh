       ...
       if [ $topath -eq 1 ]; then
         ...
         if [ $extension == "B" ] || [ $extension == "K" ]; then
           if [ -n "${DYLD_LIBRARY_PATH:+x}" ]; then
             echo "export DYLD_LIBRARY_PATH=$search/lib:$DYLD_LIBRARY_PATH" >> $file
           else
             echo "export DYLD_LIBRARY_PATH=$search/lib" >> $file
           fi
         else
           if [ -n "${DYLD_LIBRARY_PATH:+x}" ]; then
             echo "setenv DYLD_LIBRARY_PATH $search/lib:$DYLD_LIBRARY_PATH" >> $file
           else
         ...
         if [ $extension == "B" ] || [ $extension == "K" ]; then
           if [ -n "${LD_LIBRARY_PATH:+x}" ]; then
             echo "export LD_LIBRARY_PATH=$search/lib:$LD_LIBRARY_PATH" >> $file
           else
             echo "export LD_LIBRARY_PATH=$search/lib" >> $file
           fi
         else
           if [ -n "${LD_LIBRARY_PATH:+x}" ]; then
             echo "setenv LD_LIBRARY_PATH $search/lib:$LD_LIBRARY_PATH" >> $file
           else
